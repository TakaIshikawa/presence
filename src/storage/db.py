...
class Database:
    ...
    def get_last_github_pr_review_poll_time(self) -> Optional[datetime]:
        """Get the last successful GitHub PR review comment poll time."""
        value = self.get_meta("github_pr_reviews:last_poll_time")
        if not value:
            return None
        return datetime.fromisoformat(value)

    def set_last_github_pr_review_poll_time(self, poll_time: datetime) -> None:
    ...
    def get_last_github_pr_comment_poll_time(self) -> Optional[datetime]:
        """Get the last successful combined GitHub PR comment poll time."""
        value = self.get_meta("github_pr_comments:last_poll_time")
        if not value:
            return None
        return datetime.fromisoformat(value)

    def set_last_github_pr_comment_poll_time(self, poll_time: datetime) -> None:
        """Update the last successful combined GitHub PR comment poll time."""
        self.set_meta("github_pr_comments:last_poll_time", poll_time.isoformat())
    ...
    def get_last_github_discussion_comment_poll_time(self) -> Optional[datetime]:
        """Get the last successful GitHub discussion comment poll time."""
        value = self.get_meta("github_discussion_comments:last_poll_time")
        if not value:
            return None
        return datetime.fromisoformat(value)

    def set_last_github_discussion_comment_poll_time(self, poll_time: datetime) -> None:
        """Update the last successful GitHub discussion comment poll time."""
        self.set_meta("github_discussion_comments:last_poll_time", poll_time.isoformat())
