#!/usr/bin/env python3
"""Report publication timezone and local-day drift."""

from __future__ import annotations

import argparse
import sqlite3
import sys
from pathlib import Path

sys.path.insert(0, str(Path(__file__).parent.parent / "src"))

from evaluation.publication_timezone_drift import (  # noqa: E402
    DEFAULT_DAYS,
    DEFAULT_TIMEZONE_OFFSET_HOURS,
    build_publication_timezone_drift_report,
    format_publication_timezone_drift_json,
    format_publication_timezone_drift_text,
)
from runner import script_context  # noqa: E402


def _positive_int(value: str) -> int:
    try:
        parsed = int(value)
    except ValueError as exc:
        raise argparse.ArgumentTypeError(f"invalid integer: {value}") from exc
    if parsed <= 0:
        raise argparse.ArgumentTypeError("value must be positive")
    return parsed


def _timezone_offset(value: str) -> int:
    try:
        parsed = int(value)
    except ValueError as exc:
        raise argparse.ArgumentTypeError(f"invalid integer: {value}") from exc
    if parsed < -23 or parsed > 23:
        raise argparse.ArgumentTypeError("value must be between -23 and 23")
    return parsed


def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--db", help="SQLite database path. Defaults to configured database.")
    parser.add_argument("--days", type=_positive_int, default=DEFAULT_DAYS)
    parser.add_argument("--platform", default="all")
    parser.add_argument(
        "--timezone-offset-hours",
        type=_timezone_offset,
        default=DEFAULT_TIMEZONE_OFFSET_HOURS,
        help="Local publication timezone offset from UTC in whole hours.",
    )
    parser.add_argument("--format", choices=("text", "json"), default="text")
    return parser.parse_args(argv)


def main(argv: list[str] | None = None) -> int:
    try:
        args = parse_args(argv)
    except SystemExit as exc:
        return int(exc.code or 0)

    try:
        if args.db:
            with sqlite3.connect(args.db) as conn:
                conn.row_factory = sqlite3.Row
                report = build_publication_timezone_drift_report(
                    conn,
                    days=args.days,
                    platform=args.platform,
                    timezone_offset_hours=args.timezone_offset_hours,
                )
        else:
            with script_context() as (_config, db):
                report = build_publication_timezone_drift_report(
                    db,
                    days=args.days,
                    platform=args.platform,
                    timezone_offset_hours=args.timezone_offset_hours,
                )
    except (OSError, sqlite3.Error, TypeError, ValueError) as exc:
        print(f"error: {exc}", file=sys.stderr)
        return 1

    if args.format == "json":
        print(format_publication_timezone_drift_json(report))
    else:
        print(format_publication_timezone_drift_text(report))
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
