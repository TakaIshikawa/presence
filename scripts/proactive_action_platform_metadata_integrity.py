#!/usr/bin/env python3
"""Report proactive action platform metadata integrity gaps."""
...
from engagement.proactive_action_platform_metadata_integrity import (  # noqa: E402
    DEFAULT_ACTION_TYPE,
    DEFAULT_DAYS,
    DEFAULT_LIMIT,
    DEFAULT_STATUS,
    ...
    try:
        if args.db:
            with sqlite3.connect(args.db) as conn:
                ...
                report = build_proactive_action_platform_metadata_integrity_report_from_db(conn, **kwargs)
        else:
            with script_context() as (_config, db):
                report = build_proactive_action_platform_metadata_integrity_report_from_db(db, **kwargs)
    except (OSError, sqlite3.Error, TypeError, ValueError) as exc:
        print(f"error: {exc}", file=sys.stderr)
        return 1
    print(
        format_proactive_action_platform_metadata_integrity_text(report)
        if args.format == "text"
        else format_proactive_action_platform_metadata_integrity_json(report)
    )
    return 0
