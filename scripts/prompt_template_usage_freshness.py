...
from evaluation.prompt_template_usage_freshness import (  # noqa: E402
    DEFAULT_LIMIT,
    DEFAULT_LOOKBACK_DAYS,
    DEFAULT_STALE_DAYS,
    build_prompt_template_usage_freshness_report_from_db,
    format_prompt_template_usage_freshness_json,
...
def main(argv: list[str] | None = None) -> int:
    try:
        ...
        with script_context() as (_config, db):
            report = build_prompt_template_usage_freshness_report_from_db(
                db,
        ...
        return 1
    print(
        format_prompt_template_usage_freshness_text(report)
        if args.table or args.format == "text"
        else format_prompt_template_usage_freshness_json(report)
    )
    return 0
