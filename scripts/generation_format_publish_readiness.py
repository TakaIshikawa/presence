...
from evaluation.generation_format_publish_readiness import (  # noqa: E402
    DEFAULT_DAYS,
...
def main(argv: list[str] | None = None) -> int:
    try:
        ...
        with script_context() as (_config, db):
            report = build_generation_format_publish_readiness_report(db, days=args.days, min_eval_score=args.min_eval_score)
    except (OSError, sqlite3.Error, TypeError, ValueError) as exc:
        print(f"error: {exc}", file=sys.stderr)
        return 1
    as_text = args.table or args.format == "text"
    print(format_generation_format_publish_readiness_text(report) if as_text else format_generation_format_publish_readiness_json(report))
    return 0
