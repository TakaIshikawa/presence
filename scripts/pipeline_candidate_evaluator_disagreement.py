...
def parse_args(argv=None):
    p=argparse.ArgumentParser(description=__doc__); p.add_argument('--db'); p.add_argument('--min-score-spread',type=float,default=DEFAULT_MIN_SCORE_SPREAD); p.add_argument('--since'); p.add_argument('--format',choices=('json','text'),default='json'); return p.parse_args(argv)
def main(argv=None):
    ...
    try:
        ...
        if a.db:
            ...
            with script_context() as (_cfg,db): r=build_pipeline_candidate_evaluator_disagreement_report_from_db(db,**kw)
