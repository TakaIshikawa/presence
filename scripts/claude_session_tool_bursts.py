...
from ingestion.claude_session_tool_bursts import (  # noqa: E402
    DEFAULT_MIN_TOOLS,
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help=f"Fixed burst window size in minutes (default: {DEFAULT_WINDOW_MINUTES}).",
    )
    parser.add_argument(
        "--min-tools",
        type=_positive_int,
        default=DEFAULT_MIN_TOOLS,
        help=f"Minimum tool events required in a window (default: {DEFAULT_MIN_TOOLS}).",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(format_claude_session_tool_bursts_json(report))
    return 0
