...
def _share(v):
    ...
    if x<0 or x>1: raise argparse.ArgumentTypeError("value must be between 0 and 1")
    return x
def parse_args(argv=None):
    p=argparse.ArgumentParser(description=__doc__); p.add_argument('--db'); p.add_argument('--format',choices=('json','text'),default='json'); p.add_argument('--lookback-days',type=_pos,default=DEFAULT_LOOKBACK_DAYS); p.add_argument('--similarity-threshold',type=_share,default=DEFAULT_SIMILARITY); return p.parse_args(argv)
def main(argv=None):
    ...
    try:
        ...
        if a.db:
            ...
            with script_context() as (_cfg,db): r=build_newsletter_subject_line_reuse_report_from_db(db,**kw)
