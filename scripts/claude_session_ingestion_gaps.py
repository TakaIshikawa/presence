...
from evaluation.claude_session_ingestion_gaps import (  # noqa: E402
    DEFAULT_LIMIT,
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--project-path", help="Restrict findings to one Claude project path.")
...
def main(argv: list[str] | None = None) -> int:
    try:
        ...
        with script_context() as (_config, db):
            report = build_claude_session_ingestion_gaps_report(
                db,
        ...
        return 1
    if args.table or (not args.json and args.format == "text"):
        print(format_claude_session_ingestion_gaps_text(report))
    else:
        print(format_claude_session_ingestion_gaps_json(report))
    return 0
