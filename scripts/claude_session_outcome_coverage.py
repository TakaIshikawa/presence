#!/usr/bin/env python3
"""Report Claude Code session downstream outcome coverage."""
...
from ingestion.claude_session_outcome_coverage import (  # noqa: E402
    DEFAULT_DAYS,
    DEFAULT_LIMIT,
    STATUSES,
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--status",
        choices=STATUSES,
        help="Restrict to sessions with one outcome status.",
    )
    parser.add_argument(
        "--limit",
        type=_positive_int,
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.format == "json":
        print(format_claude_session_outcome_coverage_json(report))
    else:
        print(format_claude_session_outcome_coverage_text(report))
    return 0
