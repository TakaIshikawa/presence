#!/usr/bin/env python3
"""Report stale newsletter link metadata before send."""
...
from evaluation.newsletter_link_freshness import (  # noqa: E402
    DEFAULT_DAYS,
    DEFAULT_LIMIT,
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        if args.db:
            with sqlite3.connect(args.db) as conn:
                ...
                report = build_newsletter_link_freshness_report(
                    conn,
                    days=args.days,
                    limit=args.limit,
                )
    ...
    if args.json:
        print(format_newsletter_link_freshness_json(report))
    else:
        print(format_newsletter_link_freshness_text(report))
    return 0
