...
from evaluation.prompt_template_coverage_gap import (  # noqa: E402
    DEFAULT_DAYS,
    DEFAULT_LIMIT,
    build_prompt_template_coverage_gap_report_from_db,
    format_prompt_template_coverage_gap_json,
...
def main(argv: list[str] | None = None) -> int:
    try:
        ...
        with script_context() as (_config, db):
            report = build_prompt_template_coverage_gap_report_from_db(db, days=args.days, limit=args.limit)
    except (OSError, sqlite3.Error, TypeError, ValueError) as exc:
        print(f"error: {exc}", file=sys.stderr)
        return 1

    print(
        format_prompt_template_coverage_gap_text(report)
        if args.table or args.format == "text"
        else format_prompt_template_coverage_gap_json(report)
    )
    return 0
